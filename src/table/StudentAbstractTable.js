      ...
      <tr key={id}>
        ...
        <td>
          <BadgeRemainingTickets
            id={id}
            targetIso8601={moment("2020-08-31").date(1).format("YYYY-MM-DD")}
          />
        </td>
        <td style={{ borderLeft: "1px solid gray" }}>
          <BadgeRemainingTickets
            id={id}
            targetIso8601={moment("2020-08-31")
              .add(1, "month")
              .date(1)
              .format("YYYY-MM-DD")}
          />
      ...
      <table className={"bd-" + membershipFilter}>
        <thead>
          <tr>
            ...
            <th>年齢区分</th>
            <th>ステータス</th>
            <th>{moment("2020-08-31").month() + 1}月利用可能なチケット</th>
            <th>
...
const simplifyAge = birthdate => {
  ...
  const age = moment("2020-08-31").diff(moment(birthdate), "years");
